...
access_poins_info::access_poins_info()
{
    ...
    for (auto it = scan_info.cbegin(); it != scan_info.cend(); ++it)
    {
        ...
        auto pos = std::find_if(scan_info.begin(), scan_info.end(), [&ssid, it](const wifi_ap_record &e)
                                { return ssid == e.ssid_string(); });
    ...
    std::sort(begin(), end(), [](const wifi_ap_record &first, const wifi_ap_record &second)
